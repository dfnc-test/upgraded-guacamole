...
def fetch_data():
    ...
    id_to_type = {item["id"]: item.get("type", "") for item in mapping}  # for high-volume categories
...
# ---------- SPECULATIVE TRADES ----------
def speculative_trades(prices, volumes, names, lookback_days=30):
    rows = []
    for item_id, data in prices.items():
        item_id = int(item_id)
        low = data.get("low")
        ...
        try:
            ...
            if not hist_data:
                continue
            prices_list = list(hist_data.values())[-lookback_days:]
            avg_price = np.mean(prices_list)
            if low < avg_price*0.85:  # 15% below historical avg
    ...
    if df.empty:
        return df
    return df.sort_values(by="Potential Margin", ascending=False).head(20)

# ---------- UI ----------
st.set_page_config(page_title="OSRS GE Advanced Dashboard", layout="centered")
st.title("📊 OSRS GE Advanced Dashboard")
...
if regular_df.empty:
    st.warning("No regular profitable trades found.")
else:
...
if high_vol_df.empty:
    st.warning("No high-volume low-margin items found.")
else:
...
if spec_df.empty:
    st.warning("No speculative trades found.")
else:
