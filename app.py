...
def analyze_items(prices, volumes, names, capital):
    rows = []
    ...
    for item_id, data in prices.items():
        ...
        high = data.get("high")
        low = data.get("low")
        vol = volumes.get(str(item_id), {}).get("volume", 0)
    ...
    return normal.head(15), high_value.head(10)
...
normal, high_value = analyze_items(prices, volumes, names, capital)
