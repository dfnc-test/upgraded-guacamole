...
def render_trade_card(row: dict, save_key: str | None = None):
    ...
    with info_col:
        ...
        buy     = "{:,} gp".format(row["Buy"])
        sell    = "{:,} gp".format(row["Sell"])
        margin  = "{:,} gp".format(row["Margin"])
        roi     = "{} %".format(row["ROI %"])
        gphr    = "{:,}".format(row["GP/hr"])
        vol     = "{:,}".format(row["Volume (24h)"])
        limit   = str(row["Buy Limit"])
        size    = "{:,}".format(row["Trade Size"])
        fill    = "{} h".format(row["Fill (hrs)"])
        stab    = "{} %".format(int(row["Stability"] * 100))
    ...
    if not saved:
        ...
        for i, t in enumerate(saved):
            ...
            with info_col:
                ...
                s_buy    = "{:,} gp".format(t["buy_at"])
                s_sell   = "{:,} gp".format(t["sell_at"])
                s_tax    = "{:,} gp".format(t["tax"])
                s_margin = "{:,} gp".format(t["margin"])
                s_roi    = "{} %".format(t["roi_pct"])
                s_size   = "{:,}".format(t["trade_size"])
                s_profit = "{:,} gp".format(t["profit_flip"])
                st.markdown(
                    stat("Bought at",   s_buy)    + " &nbsp;&nbsp; " +
                    stat("Target sell", s_sell)   + " &nbsp;&nbsp; " +
                    stat("Tax",         s_tax)    + " &nbsp;&nbsp; " +
                    stat("Margin",      s_margin) + " &nbsp;&nbsp; " +
                    stat("ROI",         s_roi),
                    unsafe_allow_html=True,
                )
                st.markdown(
                    stat("Size",         s_size)   + " &nbsp;&nbsp; " +
                    stat("Total profit", s_profit),
                    unsafe_allow_html=True,
                )
            with del_col:
